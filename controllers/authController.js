...
const UserRegisterController = async (req, res) => {
    try {
        ...
        if (!email || !password || !name || !age || !level) {
            return res.status(400).json({ message: "Email, password, name, age, and level are required" });
        }
        ...
        const userUuid = await userQueries.registerUser(email, hashedPassword, name, age,level);
